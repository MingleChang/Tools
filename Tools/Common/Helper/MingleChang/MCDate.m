...
-(NSInteger)monthsFrom:(MCDate *)date{
    ...
    NSDate *latest = (earliest == self.date) ? date.date : self.date;
    NSInteger multiplier = (earliest == self.date) ? -1 : 1;
    NSDateComponents *components = [self.calendar components:NSCalendarUnitWeekday fromDate:earliest toDate:latest options:0];
    return multiplier*components.weekday;
}
-(NSInteger)weeksFrom:(MCDate *)date{
    NSDate *earliest = [self.date earlierDate:date.date];
    NSDate *latest = (earliest == self.date) ? date.date : self.date;
    NSInteger multiplier = (earliest == self.date) ? -1 : 1;
    NSDateComponents *components = [self.calendar components:NSCalendarUnitMonth fromDate:earliest toDate:latest options:0];
    return multiplier*components.month;
}
